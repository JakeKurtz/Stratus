...
//uniform float cld_shape_intsty2;
...
vec3 atmo_raymarch(Ray ray, float ray_length, out vec3 opacity) 
{
	...
	return ((in_scatter_ray * beta_ray * phase_ray) + (in_scatter_mie * (mix(vec3(0.0), beta_mie, foo)) * phase_mie)) * (light_intsty2);
}
...
vec3 cloud_ray_light(Ray ray) 
{
    ...
    light_ray.o = ray.o;
    light_ray.d = light_dir;
    vec3 light_color = direct_light_color(light_ray)*light_intsty2;
...
{       
    ...
    Ray ray;
    ray.o = vec3(0.0);
    ray.d = sample_spherical_direction(uv);
