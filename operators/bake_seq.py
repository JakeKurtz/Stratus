...
class STRATUS_OT_bake_seq (bpy.types.Operator):
    bl_idname = "stratus._bake_seq"
    bl_label = "Stratus Bake Sequence"
    ...
    def _pre_draw_callback(self, context):
        if not STRATUS_OT_bake_seq.validate():
            return
    ...
    def execute(self, context):
        if STRATUS_OT_bake_seq._is_enabled:
            return {'FINISHED'}
        else:
            ...
            STRATUS_OT_bake_seq._handle_add(self, context)
            STRATUS_OT_bake_seq._is_enabled = True
    ...
    def modal(self, context, event):
        ...
        if self._frame_done:
            ...
            self._env_img.save_to_disk(context, str(self._current_frame).zfill(4))
            self._env_img.reset()
    ...
    def clean_up(self, context):
        STRATUS_OT_bake_seq._is_enabled = False
